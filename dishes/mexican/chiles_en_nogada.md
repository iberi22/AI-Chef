---
title: Chiles en Nogada
region: Puebla
language: es
license: MIT
source:
  name: Gastronomic Open Standard (GOS) Database
  url: pending
  date_retrieved: '2026-09-03'
  notes: Derived from GOS Database
categories:
  - Plato fuerte
  - Patrio
  - Estacional
difficulty: ★★★★☆
prep_time: 1 hora 30 minutos
cook_time: 45 minutos
servings: 4
tags:
  - Mexican
  - GOS Catalog
  - Top 20
main_ingredients:
  - Chiles poblanos
  - Carne picada de cerdo y res
  - Nuez de Castilla fresca
  - Queso de cabra
  - Granada roja
  - Durazno
  - Manzana criolla
sensory:
  flavor:
    - Dulce salado
    - Nuez de Castilla
    - Especiado
  texture:
    - Textura suave del chile
    - Cremoso de la nogada
    - Crocante de granada
  aroma:
    - Nuez fresca
    - Jerez
    - Frutas salteadas
  presentation: >-
    Chile poblano relleno cubierto con salsa blanca de nogada, decorado con
    granos de granada roja y perejil verde.
nutrition:
  calories: 850
  macros:
    protein_g: 35
    fat_g: 42
    carbs_g: 80
images:
  - url: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600'
    description: Chiles en Nogada tradicional
description: >-
  Los Chiles en Nogada son el plato patrio por excelencia de Puebla, luciendo
  los colores de la bandera mexicana con chile poblano, crema de nuez y granada.
---
# Chiles en Nogada

Los Chiles en Nogada son el plato patrio por excelencia de Puebla, luciendo los colores de la bandera mexicana con chile poblano, crema de nuez y granada.

## Información General

* **Región:** Puebla
* **Dificultad:** ★★★★☆
* **Tiempo de preparación:** 1 hora 30 minutos
* **Tiempo de cocción:** 45 minutos
* **Porciones:** 4

---
## 📝 Ingredientes

- Chiles poblanos
- Carne picada de cerdo y res
- Nuez de Castilla fresca
- Queso de cabra
- Granada roja
- Durazno
- Manzana criolla

---

## 👨‍🍳 Instrucciones

1. **Preparación previa:** Organizar y medir todos los ingredientes principales. Limpiar las superficies de trabajo.
2. **Cocción principal:** Seguir los tiempos de cocción indicados manteniendo la temperatura controlada.
3. **Emplatado y presentación:** Servir caliente prestando especial atención a la presentación sensorial descrita.

---

## 🔬 Perfil Sensorial Estandarizado

* **Sabor:** Dulce salado, Nuez de Castilla, Especiado
* **Textura:** Textura suave del chile, Cremoso de la nogada, Crocante de granada
* **Aroma:** Nuez fresca, Jerez, Frutas salteadas
* **Presentación:** Chile poblano relleno cubierto con salsa blanca de nogada, decorado con granos de granada roja y perejil verde.
