---
title: Focaccia Barese con Pomodorini
region: Puglia (Bari)
language: es
license: MIT
source:
  name: Gastronomic Open Standard (GOS) Database
  url: pending
  date_retrieved: '2026-09-03'
  notes: Derived from GOS Database
categories:
  - Panadería
  - Snack
difficulty: ★★☆☆☆
prep_time: 30 minutos
cook_time: 30 minutos
servings: 6
tags:
  - Italian
  - GOS Catalog
  - Top 20
main_ingredients:
  - Harina de trigo de fuerza
  - Semola rimacinata
  - Patata cocida machacada
  - Tomates cherry maduros
  - Aceitunas de Bari
  - Orégano
  - Aceite de oliva extra virgen
sensory:
  flavor:
    - Aceite de oliva rico
    - Tomates cherry dulces asados
    - Orégano silvestre
  texture:
    - Exterior frito en bandeja crujiente
    - Miga interior alta esponjosa
  aroma:
    - Patata cocida en masa
    - Abundante aceite de oliva virgen extra
  presentation: >-
    Pan rectangular alto con hendiduras cubierto de tomates cherry aplastados y
    aceitunas negras.
nutrition:
  calories: 460
  macros:
    protein_g: 10
    fat_g: 18
    carbs_g: 64
images:
  - url: 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=600'
    description: Focaccia Barese con Pomodorini tradicional
description: >-
  La Focaccia Barese de Apulia incluye patata cocida en su masa para lograr una
  miga esponjosa con un exterior dorado frito en aceite de oliva y tomates
  asados.
---
# Focaccia Barese con Pomodorini

La Focaccia Barese de Apulia incluye patata cocida en su masa para lograr una miga esponjosa con un exterior dorado frito en aceite de oliva y tomates asados.

## Información General

* **Región:** Puglia (Bari)
* **Dificultad:** ★★☆☆☆
* **Tiempo de preparación:** 30 minutos
* **Tiempo de cocción:** 30 minutos
* **Porciones:** 6

---
## 📝 Ingredientes

- Harina de trigo de fuerza
- Semola rimacinata
- Patata cocida machacada
- Tomates cherry maduros
- Aceitunas de Bari
- Orégano
- Aceite de oliva extra virgen

---

## 👨‍🍳 Instrucciones

1. **Preparación previa:** Organizar y medir todos los ingredientes principales. Limpiar las superficies de trabajo.
2. **Cocción principal:** Seguir los tiempos de cocción indicados manteniendo la temperatura controlada.
3. **Emplatado y presentación:** Servir caliente prestando especial atención a la presentación sensorial descrita.

---

## 🔬 Perfil Sensorial Estandarizado

* **Sabor:** Aceite de oliva rico, Tomates cherry dulces asados, Orégano silvestre
* **Textura:** Exterior frito en bandeja crujiente, Miga interior alta esponjosa
* **Aroma:** Patata cocida en masa, Abundante aceite de oliva virgen extra
* **Presentación:** Pan rectangular alto con hendiduras cubierto de tomates cherry aplastados y aceitunas negras.
